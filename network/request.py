...
class AsyncRequest:
    ...
    async def download_site(self, session: ClientSession, url: Text) -> Page:
        """
    ...
    async def download_all_sites(self, sites: List[Text]) -> asyncio.Future[list]:
        """
    ...
    def fetch(self, sites: List[Text]) -> AsyncRequest:
        """
        ...
        """
        self.pages = self.loop.run_until_complete(self.download_all_sites(sites))
        self.loop.stop()
        return self
